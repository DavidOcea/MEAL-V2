# -*- coding: utf-8 -*-
# @Author: dingrui
# @Date:   2020-02-25 05:50:21
# @Last Modified by:   dingrui
# @Last Modified time: 2020-02-25 05:55:09
...
# __all__ = ['proxyless_cpu','proxyless_gpu','proxyless_mobile','proxyless_mobile_14']
__all__ = ['proxyless_jh_0911']
...
class ProxylessNASNets(BasicUnit):
    ...
    def __init__(self, num_classes,first_conv, blocks, feature_mix_layer, classifier):
        ...
        # self.fcs = nn.ModuleList([nn.Linear(1280, self.num_classes[k]) for k in range(self.num_tasks)])
        self.fcs = nn.Linear(1280, self.num_classes)
    ...
    def forward(self, x):
        ...
        # x = self.classifier(x)
        # x = [self.fcs[k](x) for k in range(self.num_tasks)]
        x = self.fcs(x)
        return x
    ...
    @staticmethod
    def build_from_config(config):
        num_classes = config['classifier']['out_features']
...
def proxyless_base(pretrained=False, net_config=None, net_weight=None, num_classes=[3,7,13], dropout=0):
...
def proxyless_jh_0911(num_classes):
    return proxyless_base(num_classes=num_classes,net_config = "gpu_config",net_weight = model_urls['proxyless_gpu'])

if __name__ == '__main__':
    model = proxyless_jh_0911(num_classes=8)
    print(model)
